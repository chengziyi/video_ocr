...
class Ocr(object):
    ...
    def find_all_cs(self, ocr_str, text_str, drop_short=True):
    ...
    def lcs(self, ocr_str, text_str):
        ...
        ## 取所有公共子串，返回第一个和最后一个
        ## OCR_v2.py：取所有公共子串的第一个和最后一个，如果第一个和最后一个都不在开头结尾则忽略
        if len(ocr_str)>=6:
            start_end, max_length = self.find_all_cs(ocr_str, text_str)
        else:
            start_end, max_length = self.find_all_cs(ocr_str, text_str, drop_short=False)
        sub_start,sub_end = start_end.split('__')
        if text_str.count(sub_start)>1 and text_str.count(sub_end)>1:
            print(f'common str more than 1, {sub_start} {sub_end}')
            return sub_str,0
        str_start = text_str.find(sub_start)
        str_end = text_str.rfind(sub_end) + len(sub_end)

        if 25<=str_start<=len(text_str)-25 and 25<=str_end<=len(text_str)-25:
